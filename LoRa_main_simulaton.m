...
for k = 1:length(EbNo_vec)
    ...
    % receive signal
    snr_db = EbNo_vec(k) + 10*log10(code_rate/4) + 10*log10(SF); 

    Pr = N_noise + snr_db; 
    t_range = (10 .^ ((Pr - Pt - Gr - Gt) / 20)) * 4 * pi / lambda;

    recived_message = LoRa_Rx(signal_mod, BW, SF, Coherence, Fs, df, snr_db, Preamble_Symbol_number);
