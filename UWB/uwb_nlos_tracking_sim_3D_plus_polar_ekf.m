...
% Features: 3D Kinematics, Selectable Filter Domain ('cartesian' vs 'polar'),
%           Selectable Gaussian / Bursty Noise, Sage-Husa Adaptive R, Huber EKF
% =========================================================================
clear; clc; close all;
...
%% 4. Filter Setup (Selectable Cartesian vs. Polar Extended Kalman Filter)
...
if strcmpi(filter_type, 'cartesian')
    ...
    pA_0 = posA_gps(:, 1);
    rel_0 = posB_true(:, 1) - pA_0; % Initial relative vector guess
    r_0     = norm(rel_0);
...
for k = 1:N
    ...
    if strcmpi(filter_type, 'cartesian')
        % Cartesian Mode: Non-linear range function
        dx = x_pred(1) - pA(1);
        dy = x_pred(2) - pA(2);
        dz = x_pred(3) - pA(3);
        ...
        % Polar Mode: Linear measurement model for range
        dist_pred = x_pred(1);
        H = [1, 0, 0, 0, 0, 0];
        y = dist_uwb(k) - dist_pred;
    end
    ...
    if dist_pred > 1e-4
        ...
        if std_residual <= huber_c
            ...
            w = huber_c / std_residual;          % Downweight bursts/outliers
        end
    ...
    if strcmpi(filter_type, 'cartesian')
        ...
        % Convert Spherical relative state [r, theta, phi] back to Cartesian Tag B
        r_e     = x_est(1);
        theta_e = x_est(2);
        phi_e   = x_est(3);
...
figure('Name', sprintf('3D %s EKF - [%s Noise]', upper(filter_type), upper(noise_type)), ...
...
plot3(posB_est(1,:), posB_est(2,:), posB_est(3,:), 'r:', 'LineWidth', 2, 'DisplayName', sprintf('Tag B %s EKF', upper(filter_type)));
xlabel('X Position (m)'); ylabel('Y Position (m)'); zlabel('Z Position (m)');
title(sprintf('3D Trajectory Tracking (%s Domain)', upper(filter_type)));
